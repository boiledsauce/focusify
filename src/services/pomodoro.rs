...
impl PomodoroTimer {
    ...
    pub async fn start(&self) {
        loop {
            ...
            // Once every second, update the self.remaining time
            let mut remaining_time: tokio::sync::MutexGuard<'_, Duration> =
                self.remaining.lock().await;

            // If the remaining time is 0, handle the state transition
